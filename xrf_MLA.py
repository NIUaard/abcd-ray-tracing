...
Spaces = [0.1, 0.1, 0.2, 0.2, 0.36, 0.27, 0.45]
zL = np.cumsum(Spaces)
print ('positions:', zL)

# specify elements and focal lengths
Elem   = ["MLA","MLA","L","D","L","L","D"]
#focL   = [0.05,0.20,0]
focL = [0, 0, 0.5, 0, 0.04, 0.15, 0]
...
N     = 100000
# beam size in m
sigx  = [20e-6,50e-6,75e-6,100e-6,125e-6,150e-6,175e-6,200e-6,5e-3]
   ...
   for i in range(len(Elem)):
      if (zinit<=zL[i]) & (zinit+dz>zL[i]):
        ...
        if Elem[i]=="MLA":
           print("------------------------ MLA")
           print("position [m]=",zL[i])
           print("aperture [m]=       ",focMLA)
           Xfin=ABCD.MLA(focMLA,pitch, radius, thickness, num, X1)
